...
void ldgLow(){
  ...
  LDG_KEY_ACTIVE = true;
  //mode_prev = mode_actual;
  if(mode_actual == ENABLE)
  {
    //mode_actual = TUNE;
    SET_IC_TKEY(true);
  }
  attachInterrupt(digitalPinToInterrupt(LDG_KEY), ldgHigh, HIGH);
}

void ldgHigh(){
  Serial.println("LDG_KEY HIGH Interrpt");
  LDG_KEY_ACTIVE = false;
  //mode_actual = mode_prev;
  SET_IC_TKEY(false);
  attachInterrupt(digitalPinToInterrupt(LDG_KEY), ldgLow, LOW);
}
...
///////   if(digitalRead(9) == LOW) performResync();  //testing
 ...
 if(trigd && pulse > 55)
 {
   ...
   Serial.println(pulse);
    ...
    //if(valid){
    //  valid = false;
   // detachInterrupt(digitalPinToInterrupt(IC_TSTART));
    ...
    switch(mode_req)
    {
      ...
      if(mode_actual == -1) 
      {
        ...
        /*
        else if(mode_actual == TUNE)
        {
          Serial.println("CASE: DISABLE> MODE IS TUNE");
          // Wait for TUNE to finish, or abort tune (if possible?) by immediately sending disable pulse to LDG //
          while(LDG_KEY_ACTIVE)
          {
            Serial.println("Awaiting TUNE to complete...");
            delay(250);
          }
          LDG_command(TOGGLE);
          mode_actual = DISABLE;
